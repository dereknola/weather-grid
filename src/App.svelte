<script lang="ts">
  ...
  import { Flame, LoaderCircle, MapPinPlus, RadioTower, RefreshCw, Search, Thermometer, Trash2, Umbrella, Wind } from '@lucide/svelte';
...
<main class="shell">
  ...
  <section class="control-deck" aria-label="Weather grid controls">
    ...
    <div class="settings-panel"><div class="setting-group"><span class="control-label">Show on cards</span><div class="check-row">{#each [['current', 'Current'], ['daily', '5 day'], ['hourly', 'Hourly'], ['stations', 'Station']] as [key, label] (key)}<label class="check-control"><input type="checkbox" checked={grid.settings[key as 'current' | 'daily' | 'hourly' | 'stations']} onchange={() => grid.toggleSection(key as 'current' | 'daily' | 'hourly' | 'stations')} /><span>{label}</span></label>{/each}</div></div><div class="setting-group units-group"><span class="control-label">Temperature</span><div class="unit-switch" role="group" aria-label="Temperature unit"><button class:active={grid.settings.temperatureUnit === 'celsius'} type="button" onclick={() => grid.setUnit('celsius')}>°C</button><button class:active={grid.settings.temperatureUnit === 'fahrenheit'} type="button" onclick={() => grid.setUnit('fahrenheit')}>°F</button></div></div><button class="refresh-button" type="button" onclick={grid.refreshAll} aria-label="Refresh all forecasts" title="Refresh all forecasts"><RefreshCw size={17} /></button></div>
  </section>
