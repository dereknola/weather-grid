<script lang="ts">
  ...
  import { Flame, LoaderCircle, MapPinPlus, RefreshCw, Search, Thermometer, Trash2, Umbrella, Wind } from '@lucide/svelte';
...
<main class="shell">
  <header class="masthead"><div class="brand-lockup"><div class="brand-mark" aria-hidden="true"><Wind size={22} strokeWidth={1.8} /></div><div><p class="eyebrow">Personal forecast system</p><h1>Weather <em>Grid</em></h1></div></div><p class="date-note">One glance.<br /><strong>Everywhere.</strong></p></header>

  <section class="control-deck" aria-label="Weather grid controls">
    <div class="search-wrap"><label for="location-search">Add a location</label><div class="search-field"><Search size={18} aria-hidden="true" /><input id="location-search" type="search" role="combobox" aria-autocomplete="list" aria-controls="location-results" aria-expanded={results.length > 0} placeholder="Search city or postcode" bind:value={query} oninput={handleSearchInput} onkeydown={handleSearchKeydown} />{#if searching}<LoaderCircle class="spin" size={18} aria-label="Searching" />{/if}</div>
      {#if results.length > 0}<ul id="location-results" class="search-results" role="listbox">{#each results as result, index}<li role="option" aria-selected={index === highlighted}><button class:highlighted={index === highlighted} type="button" onclick={() => chooseLocation(result)}><MapPinPlus size={17} aria-hidden="true" /><span><strong>{result.name}</strong><small>{[result.admin1, result.country].filter(Boolean).join(' · ')}</small></span></button></li>{/each}</ul>{:else if query.length >= 2 && !searching && searchError}<p class="search-message error-text">{searchError}</p>{:else if query.length >= 2 && !searching}<p class="search-message">No locations found.</p>{/if}
    </div>
    <div class="settings-panel"><div class="setting-group"><span class="control-label">Show on cards</span><div class="check-row">{#each [['current', 'Current'], ['daily', '5 day'], ['hourly', 'Hourly']] as [key, label]}<label class="check-control"><input type="checkbox" checked={grid.settings[key as 'current' | 'daily' | 'hourly']} onchange={() => grid.toggleSection(key as 'current' | 'daily' | 'hourly')} /><span>{label}</span></label>{/each}</div></div><div class="setting-group units-group"><span class="control-label">Temperature</span><div class="unit-switch" role="group" aria-label="Temperature unit"><button class:active={grid.settings.temperatureUnit === 'celsius'} type="button" onclick={() => grid.setUnit('celsius')}>°C</button><button class:active={grid.settings.temperatureUnit === 'fahrenheit'} type="button" onclick={() => grid.setUnit('fahrenheit')}>°F</button></div></div><button class="refresh-button" type="button" onclick={grid.refreshAll} aria-label="Refresh all forecasts" title="Refresh all forecasts"><RefreshCw size={17} /></button></div>
  </section>
