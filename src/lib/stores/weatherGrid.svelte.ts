...
import type { DisplaySettings, Forecast, Location, TemperatureUnit } from '../types/weather';

const STORAGE_KEY = 'weather-grid:v1';

export type CardState = { forecast?: Forecast; loading: boolean; error?: string };
export type WeatherGridState = {
  ...
  toggleSection: (section: 'current' | 'daily' | 'hourly') => void;
};

const defaultSettings: DisplaySettings = { current: true, daily: true, hourly: true, temperatureUnit: 'fahrenheit' };
...
function loadSavedState(): { locations: Location[]; settings: DisplaySettings } {
  ...
  try {
    ...
    return { locations, settings: { current: Boolean(settings.current), daily: Boolean(settings.daily), hourly: Boolean(settings.hourly), temperatureUnit: settings.temperatureUnit === 'fahrenheit' ? 'fahrenheit' : 'celsius' } };
...
export function createWeatherGrid(): WeatherGridState {
  ...
  function loadForecast(location: Location): void {
    ...
    cards[location.id] = { ...cards[location.id], loading: true, error: undefined };
    fetchForecast(location, settings.temperatureUnit).then((forecast) => {
      if (requestIds.get(location.id) === requestId) cards[location.id] = { forecast, loading: false };
    }).catch((error: unknown) => {
  ...
  function toggleSection(section: 'current' | 'daily' | 'hourly'): void {
    const enabledCount = Number(settings.current) + Number(settings.daily) + Number(settings.hourly);
    if (settings[section] && enabledCount === 1) return;
